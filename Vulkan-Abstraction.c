...
int vka_create_instance(vka_vulkan_t *vulkan)
{
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not create Vulkan instance.");
		vulkan->instance = VK_NULL_HANDLE;
...
int vka_create_device(vka_vulkan_t *vulkan)
{
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not create device.");
		free(queue_info);
		return -1;
	}
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not retrieve graphics queue.");
		return -1;
	}
...
int vka_create_command_pool(vka_vulkan_t *vulkan)
{
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not create command pool.");
		return -1;
	}
...
int vka_create_command_buffers(vka_vulkan_t *vulkan)
{
	...
	{
		snprintf(vulkan->command_buffers[i].name, VKA_MAX_NAME_LENGTH, "Vulkan Base %u", i);
...
int vka_create_swapchain(vka_vulkan_t *vulkan)
{
	...
	if (num_formats < 1)
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "No surface formats to choose from.");
		free(formats);
		return -1;
	}
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not create swapchain.");
	...
	for (uint32_t i = 0; i < vulkan->num_swapchain_images; i++)
	{
		snprintf(vulkan->swapchain_images[i].name, VKA_MAX_NAME_LENGTH,
						"Swapchain image %u", i);
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not get swapchain images.");
		free(images);
		return -1;
	}
	for (uint32_t i = 0; i < vulkan->num_swapchain_images; i++)
...
{
	vka_image_t *image = (vka_image_t *)(barrier_info->resource);
...
void vka_copy_buffer_to_image(vka_command_buffer_t *command_buffer, vka_copy_info_t *copy_info)
{
	...
	barrier_info.resource		= destination;
...
void vka_copy_buffer(vka_command_buffer_t *command_buffer, vka_copy_info_t *copy_info)
{
	...
	if (!copy_size)
	{
		...
		if (source->size < copy_size) { copy_size = source->size; }
	}
	...
	vkCmdCopyBuffer(command_buffer->buffer, source->buffer,
...
{
	// Uses destination buffer and offset in copy_info.
	vka_buffer_t *destination = (vka_buffer_t *)(copy_info->destination);
	VkDeviceSize size = copy_info->size;
	if (!size) { size = VK_WHOLE_SIZE; }
	vkCmdFillBuffer(command_buffer->buffer, destination->buffer,
		copy_info->destination_offset, size, copy_info->data);
}
...
int vka_check_instance_layer_extension_support(vka_vulkan_t *vulkan)
{
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not get instance layers.");
		free(layers);
		return -1;
	}
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not get instance extensions.");
		free(extensions);
		return -1;
	}
...
int vka_create_debug_messenger(vka_vulkan_t *vulkan)
{
	...
	{
		snprintf(vulkan->error, VKA_MAX_ERROR_LENGTH, "Could not create debug messenger.");
		return -1;
	}
